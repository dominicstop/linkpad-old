...
const { set, cond, eq, or, add, sub, pow, min, max, debug, multiply, divide, lessThan, spring, defined, decay, timing, call, diff, acc, not, abs, block, startClock, stopClock, clockRunning, Value, Clock, event } = Animated;
...
const VIEWER_HEIGHT = Dimensions.get('window').height - (Header.HEIGHT + StatusBar.currentHeight);

const WIDTH = Dimensions.get('window').width;
const HEIGHT = Platform.select({
  ios: 300,
  android: VIEWER_HEIGHT
});
...
const ViewImageHeader = (props) => <CustomHeader
  name={'image'}
  type={'font-awesome'}
  iconSize={22}
  color={'white'}
...
export class ViewImageScreen extends React.Component {
  static navigationOptions = {
    title: 'View Image',
    headerTitle: ViewImageHeader,
    //custom android header
    ...Platform.select({
      android: { header: props => <AndroidHeader {...{titleIcon, ...props}}/> }
    }),
  };
  ...
  constructor(props){
    ...
    const imageURI = navigation.getParam('imageBase64', null);
